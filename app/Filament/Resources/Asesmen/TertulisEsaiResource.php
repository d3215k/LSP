...
use App\Filament\Resources\Asesmen\TertulisEsaiResource\Pages;
use App\Filament\Resources\Asesmen\TertulisEsaiResource\RelationManagers;
...
class TertulisEsaiResource extends Resource
{
    protected static ?string $model = TertulisEsai::class;

    protected static ?string $cluster = Asesmen::class;

    protected static ?string $modelLabel = "Asesmen Tertulis Esai";

    protected static ?string $navigationIcon = 'heroicon-o-document-text';

    protected static ?string $navigationLabel = 'FR.IA.06';
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->recordUrl(
                fn (TertulisEsai $record): string => route('filament.app.asesmen.resources.asesmens.pertanyaan-tertulis-esai', $record->asesmen),
            )
            ->columns([
                Tables\Columns\TextColumn::make('asesmen.asesi.nama')
                    ->sortable()
                    ->label('Asesi  / No. Reg')
                    ->description(fn (TertulisEsai $record): string => $record->asesmen->asesi->no_reg ?? '-')
                    ->searchable(),
                ...
                Tables\Columns\TextColumn::make('asesmen.asesor.nama')
                    ->label('Asesor')
                    ->sortable()
                    ->description(fn (TertulisEsai $record): string => $record->asesmen->asesor->nomor_registrasi ?? '-')
                    ->toggleable(
                        condition: true,
                        isToggledHiddenByDefault: true,
                    ),
            ...
            ->actions([
                Tables\Actions\Action::make('Reset Waktu')
                    ...
                    ->action(fn(TertulisEsai $record) => $record->resetWaktu())
                    ->hidden(fn (TertulisEsai $record): bool => $record->status === \App\Enums\AsesmenTertulisStatus::SELESAI),
                Tables\Actions\Action::make('Selesai')
                    ->icon('heroicon-m-x-circle')
                    ->iconButton()
                    ->requiresConfirmation()
                    ->action(fn(TertulisEsai $record) => $record->forceFinish())
                    ->hidden(fn (TertulisEsai $record): bool => $record->status === \App\Enums\AsesmenTertulisStatus::SELESAI),
                Tables\Actions\ActionGroup::make([
                    Tables\Actions\Action::make('Lihat')
                        ->icon('heroicon-m-document-text')
                        ->url(fn (TertulisEsai $record): string => route('filament.app.asesmen.resources.asesmens.pertanyaan-tertulis-esai', $record->asesmen)),
    ...
    {
        return [
            'index' => Pages\ListTertulisEsais::route('/'),
        ];
    }
}
